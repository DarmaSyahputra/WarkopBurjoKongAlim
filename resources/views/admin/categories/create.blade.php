    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('admin.categories.store') }}" method="POST" class="p-8">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" id="name" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 outline-none transition-all @error('name') border-rose-500 @enderror" placeholder="Contoh: Kopi, Makanan Berat" value="{{ old('name') }}">
                @error('name')
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-8 py-3 bg-orange-500 text-white rounded-xl font-bold shadow-sm shadow-orange-200 hover:bg-orange-600 transition-all">Simpan Kategori</button>
            </div>
        </form>
    </div>
</div>
