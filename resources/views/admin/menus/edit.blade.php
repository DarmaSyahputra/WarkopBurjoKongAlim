        ...
        <form action="{{ route('admin.menus.update', $menu->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 outline-none transition-all" placeholder="Contoh: Indomie Telur Kornet" value="{{ old('name', $menu->name) }}">
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 outline-none transition-all" placeholder="Contoh: 12000" value="{{ old('price', $menu->price) }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 outline-none transition-all" placeholder="Ceritakan sedikit tentang menu ini...">{{ old('description', $menu->description) }}</textarea>
                    @error('description') <p class="text-rose-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <!-- Gambar -->
                <div>
                    <label for="image" class="block text-sm font-bold text-gray-700 mb-2">Gambar Menu (Kosongkan jika tidak ingin diubah)</label>
                    ...
                    <div id="image-preview-container" class="mt-4 {{ $menu->image ? '' : 'hidden' }}">
                        <p class="text-xs text-gray-500 mb-2">Preview / Gambar Saat Ini:</p>
                        <img id="image-preview" src="{{ $menu->image ? asset('storage/menus/' . $menu->image) : '#' }}" alt="Preview" class="w-32 h-32 object-cover rounded-xl border">
                    </div>
                </div>
                ...
                <div class="flex items-center">
                    <label class="flex items-center cursor-pointer">
                        <div class="relative">
                            <input type="checkbox" name="is_available" value="1" class="sr-only" {{ old('is_available', $menu->is_available) ? 'checked' : '' }}>
            ...
            <div class="flex justify-end gap-3 pt-6 border-t">
                ...
                <button type="submit" class="px-8 py-3 bg-orange-500 text-white rounded-xl font-bold shadow-sm shadow-orange-200 hover:bg-orange-600 transition-all">Perbarui Menu</button>
            </div>
        </form>
    </div>
</div>
